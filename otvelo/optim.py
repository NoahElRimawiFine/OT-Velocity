...
Code for conditional gradient descent / Frank Wolfe algorithm
from Fused Gromov-Wasserstein OT, 
"""
import numpy as np
...
def line_search_armijo(f, xk, pk, gfk, old_fval, args=(), c1=1e-4, alpha0=0.99,scaleCost=False):
	"""
	...
	def phi(alpha1):
		fc[0] += 1
		return f(xk + alpha1 * pk, *args,scaleCost=scaleCost)
...
def do_linesearch(cost,G,deltaG,Mi,f_val,amijo=True,C1=None,C2=None,reg=None,Gc=None,constC=None,M=None,scaleCost=False):
	#Gc= st
	#G=xt
	#deltaG=st-xt
	...
	"""
	if amijo:
		alpha, fc, f_val = line_search_armijo(cost, G, deltaG, Mi, f_val, scaleCost=scaleCost)
	else:
		...
		c=cost(G, scaleCost=scaleCost) #f(xt)

		alpha=solve_1d_linesearch_quad_funct(a,b,c)
		fc=None
		f_val=cost(G+alpha*deltaG, scaleCost=scaleCost)
		
	return alpha,fc,f_val

def cg(a, b, M,reg, C1=None,C2=None, G0=None,constC=None, hC1=None, hC2=None, scaleCost=False, numItermax=500, stopThr=1e-09, verbose=False,log=False,amijo=True):
	"""
	...
	def cost(G,scaleCost):
		gw= gwlossTensor(constC,hC1,hC2,G)

		if scaleCost:
			gw_scaled=(gw-np.amin(gw))/(np.amax(gw)-np.amin(gw))
			gw=gw_scaled

		ML=np.sum(M*G)
		gwL=np.sum(gw*G)
		return (1-reg)*np.sum(ML) + reg * gwL #gwloss(constC,hC1,hC2,G)

	f_val = cost(G,scaleCost=scaleCost) #f(xt)
	...
	while loop:
		...
		Mi = M + reg * gwggrad(constC,hC1,hC2,G) #Gradient(xt)
		# set M positive
		Mi += Mi.min()
		...
		alpha, fc, f_val = do_linesearch(cost=cost,G=G,deltaG=deltaG,Mi=Mi,f_val=f_val,amijo=amijo,constC=constC,C1=C1,C2=C2,reg=reg,Gc=Gc,M=M, scaleCost=scaleCost)
...
# 	return cg(p,q,M,alpha,G0,amijo=amijo,C1=C1,C2=C2,constC=constC, hC1=hC1, hC2=hC2,**kwargs)
